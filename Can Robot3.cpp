...
/*  작성자 : 제갈도현                        날짜 : 2021년 11월 20일   */
/*                                                                     */
/*  문제 정의 :   계층적 모델링 기법을 통해 구현한 로봇을 조명을       */
/*                입히고 팔다리를 움직여 달리는 모습이 되게 수정       */
/*                                                                     */
/*  기능 :                    시점 변환                                */
/*                ↑ - 로봇의 윗쪽, ↓ - 로봇의 아래쪽                 */
/*                ← - 로봇의 왼쪽, → - 로봇의 오른쪽                 */
...
static double cx = 0, cy = 0, cz = 0, rx = 0, ry = 0, rz = 0;	// 카메라의 위치 변경
static int head = 0;	// 머리의 각도를 조절
static int shoulderL = 20, shoulderR = 20, armL = 20, armR = 20;	// 팔 관련 부위의 각도를 조절
static int legL = 0, legR = 0, kneeL = 0, kneeR = 0;	// 다리의 각도 조절

static double shoulderLy = 0, shoulderRy = 0, armLy = 0, armRy = 0;	// 달리는 모션 표현을 위한 관절 각도 조절 변수
...
void runningA(int value) {	// 달리는 모션 A 
...
void runningB(int value) {	// 달리는 모션 B, A와 B는 반대방향 움직임
	shoulderLy -= 1; armLy += 1; armLp += 0.025;
	shoulderRy += 1; armRy -= 1; armRp -= 0.025;
	legLy += 1; kneeLy -= 0.8; kneeLp -= 0.005;
	legRy -= 1; kneeRy += 0.8; kneeRp += 0.005;
...
void reset() {	// 로봇의 좌표를 초기화하여 기존 상태로 복구
	head = 0;
...
void MyKeyboard(unsigned char key, int x, int y) {
	...
	case 's':
		StopTrig = true;
		break;
	case 'F':	// F,f 입력시 로봇의 정면으로 시점 변경
	case 'f':
		cx = 0; cy = 0; cz = 0; rx = 0; ry = 0; rz = 0;
		glutPostRedisplay();
		break;
	case 'B':	// B,b 입력시 로봇의 뒷쪽으로 시점 변경
	case 'b':
		cx = 0; cy = -2.0; cz = 0; rx = 0; ry = 0; rz = -180;
		glutPostRedisplay();
		break;
	case 'R':	// R,r 입력시 로봇이 팔, 다리를 움직여 달리는 모션을 취함
	case 'r':
		StopTrig = false;
...
void MySpecial(int key, int x, int y) {
	switch (key) {
	case GLUT_KEY_UP:	// ↑ 입력시 로봇의 윗쪽으로 시점 변경
		cx = 0; cy = -0.8; cz = -1.0; rx = 0; ry = 0; rz = 0;
		glutPostRedisplay();
		break;
	case GLUT_KEY_DOWN:	// ↓ 입력시 로봇의 아랫쪽으로 시점 변경
		cx = 0; cy = -1.0; cz = 1.0; rx = 0; ry = 0; rz = 0;
		glutPostRedisplay();
		break;
	case GLUT_KEY_LEFT:	// ← 입력시 로봇의 왼쪽으로 시점 변경
		cx = -1.0; cy = -1.0; cz = 0; rx = 0; ry = 0; rz = -180;
		glutPostRedisplay();
		break;
	case GLUT_KEY_RIGHT:	// → 입력시 로봇의 오른쪽으로 시점 변경
		cx = 1.0; cy = -1.0; cz = 0; rx = 0; ry = 0; rz = -180;
		glutPostRedisplay();
		break;
	}
